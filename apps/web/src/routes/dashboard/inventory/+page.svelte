				...
				{:else}
					<ul class="divide-y-2 divide-black">
						{#each data.categories as cat (cat.id)}
							<li class="p-4 hover:bg-[var(--color-surface)] group transition-colors relative overflow-hidden">
								...
								{#if editingCategoryId === cat.id}
									<form method="POST" action="?/updateCategory" class="relative z-10 flex flex-col gap-2" use:enhance={() => {
										return async ({ update }) => {
											await update();
										...
										<input type="hidden" name="tenantId" value={data.activeTenantId} />
										<input name="name" type="text" value={cat.name} required class="w-full brutal-input py-1 px-2 text-sm" />
										<input name="description" type="text" value={cat.description || ""} class="w-full brutal-input py-1 px-2 text-xs" placeholder="Desc..." />
										<div class="flex gap-2 mt-2">
											<button type="submit" class="bg-black text-white font-mono text-xs px-3 py-1 border-2 border-black hover:bg-[var(--color-brand)]">SAVE</button>
											<button type="button" onclick={() => editingCategoryId = null} class="bg-white text-black font-mono text-xs px-3 py-1 border-2 border-black hover:bg-gray-100">CANCEL</button>
											<button formaction="?/deleteCategory" class="bg-red-500 text-white font-mono text-xs px-3 py-1 ml-auto border-2 border-red-700 hover:bg-red-600">DELETE</button>
										</div>
									</form>
								{:else}
									<button 
										type="button" 
										class="w-full text-left flex justify-between items-center cursor-pointer focus:outline-none" 
		...
		<div class="lg:col-span-8 flex flex-col gap-6">
			...
			<div class="bg-black border-2 border-black p-4 brutal-shadow flex justify-between items-center text-white">
				...
				<div class="flex gap-4 items-center">
					<input type="text" bind:value={searchQuery} placeholder="SEARCH_QUERY..." class="brutal-input text-black py-1 px-2 text-xs w-48 font-mono" />
					<button 
								...
								{#if editingProductId === prod.id}
									<tr class="bg-[var(--color-surface)] border-b-2 border-black">
										<td colspan="5" class="p-4">
											<form method="POST" action="?/updateProduct" class="grid grid-cols-12 gap-4" use:enhance={() => {
												return async ({ update }) => {
													await update();
												...
												<div class="col-span-3">
													<label class="text-[10px] font-bold text-gray-500 block mb-1">SKU</label>
													<input name="sku" type="text" value={prod.sku || ""} class="w-full brutal-input py-1 px-2 text-xs" />
												</div>
												<div class="col-span-4">
													<label class="text-[10px] font-bold text-gray-500 block mb-1">NAME</label>
													<input name="name" type="text" value={prod.name} required class="w-full brutal-input py-1 px-2 text-xs" />
												</div>
												<div class="col-span-2">
													<label class="text-[10px] font-bold text-gray-500 block mb-1">PRICE</label>
													<input name="price" type="number" step="0.01" value={prod.price} required class="w-full brutal-input py-1 px-2 text-xs" />
												</div>
												<div class="col-span-2">
													<label class="text-[10px] font-bold text-gray-500 block mb-1">STOCK</label>
													<input name="stock" type="number" value={prod.stock} required class="w-full brutal-input py-1 px-2 text-xs" />
												</div>
												<div class="col-span-12 flex justify-end gap-2 mt-2">
													<button type="button" onclick={() => editingProductId = null} class="bg-white text-black border-2 border-black font-mono text-xs px-4 py-2 hover:bg-gray-100">CANCEL</button>
													<button type="submit" class="bg-black text-white font-mono text-xs px-4 py-2 hover:bg-[var(--color-brand)] border-2 border-black">UPDATE</button>
													<button formaction="?/deleteProduct" class="bg-red-500 text-white font-mono text-xs px-4 py-2 border-2 border-red-700 hover:bg-red-600 ml-4">DELETE</button>
												</div>
											</form>
										</td>
									</tr>
								{:else}
									<tr onclick={() => editingProductId = prod.id} onkeydown={(e) => e.key === 'Enter' && (editingProductId = prod.id)} tabindex="0" class="hover:bg-[var(--color-brand)] hover:text-white transition-colors group cursor-crosshair">
